...
class DataCube(object):
    ...
    """
    Wrapper for three-dimensional folded data.
    The data order is (time, freq, phase).
    """
    ...
    def __init__(self, data, copy=False):
        ...
        Classmethods are the preferred way of making
        ...
        data: ndarray
            ...
            The folded data as a 3-dimensional array,
            in (time, freq, phase) order.

        copy: bool, optional

            If True, copy the input data. Otherwise the DataCube
            stores only a reference to 'data' to save memory
            (default: False).
        """
    ...
    def baselines(self):
        ...
        """
        2D array with shape (num_subints, num_chans)
        containing the baselines of all profiles.
        """
    ...
    def subbands(self):
        ...
        """
        Sum of the data along the time axis.
        Output data order is (time, phase).
        """
    ...
    def subints(self):
        ...
        """
        Sum of the data along the frequency axis.
        Output data order is (freq, phase).
        """
    ...
    def from_psrchive(cls, archive):
        ...
        """
        Create DataCube from PSRCHIVE Archive object.
        Only Stokes I data is read.
    ...
    @classmethod
    def from_pfd(cls, pfd):

        """
        Create DataCube from a `PFD` file
        or a `PTypePFD` object.

        Parameters
        ----------
        pfd: str or `PTypePFD` object.

            The `PFD` file or `PTypePFD` object from
            which a `DataCube` has to be formed.

        Returns
        -------
        cube: DataCube

            DataCube instance wrapping folded data.
        """

        if type(pfd) == str:
            pfd = PTypePFD(pfd)

        # Data shape is (npart, nsub, nbin)
        # NOTE: Copy the data here instead
        # of giving the original.
...
def featurize(cube, features=("std", "ptp", "lfamp")):
    ...
    features: list or tuple of strings
        ...
        List of features to compute. These must name
        functions in the 'clfd.features' submodule.
    ...
    features: pandas.DataFrame
        ...
        individual features, and rows are indexed by a
        pandas.MultiIndex (subint, channel).
    ...
    Extract values for subint index 3 and channel
    indexes 15 to 19 inclusive:
    ...
    See pandas.MultiIndex documentation for advanced
    indexing/slicing.
    """

    for fn in features:
        if not hasattr(pf, fn):
            msg = "No function '{:s}' in the clfd.features sub-module."
            msg = msg.format(fn)
...
def feature_stats(features, q=2.0):
    ...
    """
    Compute quartiles, inter-quartile range and min/max
    acceptable values for every column in 'features'
    based on Tukey's rule for outliers.
    ...
    q: float, optional (default: 2.0)
        ...
        Parameter that controls the min and max values that define
        the 'inlier' or 'normality' range. For every feature, the
        first and third quartiles (Q1 and Q3) are calculated, and
        R = Q3 - Q1 is the interquartile range. The min and max
    ...
    stats: pandas.DataFrame
        ...
        DataFrame containing the results. Columns are
        individual features, and rows are individual
        statistics (quartiles, iqr, min and max values).
...
def profile_mask(features, q=2.0, zap_channels=[]):
    ...
    q: float, optional (default: 2.0)
        ...
        Parameter that controls the min and max values that
        define the 'inlier' or 'normality' range. For every
        feature, the first and third quartiles (Q1 and Q3)
        are calculated, and R = Q3 - Q1 is the interquartile
        range. The min and max acceptable values are then
        defined as:
    ...
    zap_channels: list or array of ints
        ...
        Frequency channel indices to forcibly flag as outliers.
        These are excluded from the analysis as well, and they
        will not be considered when calculating the inlier range
        for every feature.
    ...
    mask: ndarray
        ...
        A two dimensional boolean numpy array, with shape
        (num_subints, num_channels). A value of `True` means
        that the associated profile is an outlier.
    """
    ...
    mask = mask.reshape(features.index.levshape)
...
def time_phase_mask(cube, q=4.0, zap_channels=[]):
    """
    Compute a data mask based on the cube's time-phase
    plot (sum of the data along the frequency axis of
    the cube).
    ...
    q: float, optional (default: 4.0)
        ...
        Parameter that controls the min and max values that
        define the 'inlier' or 'normality' range.

    zap_channels: list or array of ints, optional (default: [])

        Frequency channel indices to exclude from the outlier
        analysis.
    ...
    mask: ndarray
        ...
        A two dimensional boolean numpy array, with shape
        (num_subints, num_bins). A value of True means that
        the associated time-phase bin is an outlier.

    valid_chans: ndarray

        List of channel indices that are not part of 'zap_channels'.
        In these valid channels, bad data values are allowed to be
        replaced.

    repvals: ndarray

        Three dimensional array with same shape as the input data
        cube, containing the appropriate replacement values for
        bad data points in the original folded data.

        A bad time-phase bin with indices (i, j) means that the
        original data `orig_data[i, valid_chans, j]` should be
        replaced by `repvals[i, valid_chans, j]`.
    """
    ...
    pcts = np.percentile(data, [25, 50, 75], axis=0)
